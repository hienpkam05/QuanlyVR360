...
const emit = defineEmits(['command-result', 'toggle-scene-list', 'toggle-theater']);
...
<template>
  <div class="yt-control-bar yt-fadeable" role="toolbar" aria-label="Điều khiển VR360">
    ...
    <div class="yt-control-row">
      <!-- LEFT: play/pause AUDIO trước (giống Youtube) + scene navigation.
           Tự động xoay đã chuyển hẳn vào menu Cài đặt (⚙) — không còn nút
           riêng ngoài thanh này nữa, tránh trùng chức năng với play/pause
           audio ở vị trí đầu tiên. -->
      ...
      <div class="yt-control-row__group">
        ...
        <!-- Chế độ rạp chiếu (theater) — mở rộng khung player trong trang,
             KHÁC với Toàn màn hình (fullscreen) bên cạnh: không chiếm cả màn
             hình thiết bị, chỉ nới khung xem trong layout trang hiện tại. -->
        <YtNavButton
          class="yt-btn--theater"
          label="Chế độ rạp chiếu"
          :active="theaterMode"
          :disabled="disabled"
          @activate="emit('toggle-theater')"
        >
          <svg viewBox="0 0 24 24" aria-hidden="true"><rect x="2.5" y="6" width="19" height="12" rx="2" /></svg>
        </YtNavButton>
